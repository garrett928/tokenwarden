...
// budget and dispatches it anyway if it's Resumable (strategy 1), promotes
// its user-authored Steps into a chain of child jobs if it has any
// (strategy 2, via queue.PromoteSteps), or defers it and tries the
// next-best candidate (strategy 4). Model-driven decomposition (§6.4
// strategy 3) is not implemented — an oversized job that is neither
// Resumable nor step-bearing always defers rather than attempting it.
// Burn-rate-based concurrency/throttling (§6.2 steps 5-6)
