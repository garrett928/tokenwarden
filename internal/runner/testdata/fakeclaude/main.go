...
func main() {
	...
	// unrecognized event type ("debug_argv"), so a test can both assert real
	// subprocess argv matches what BuildArgs computed in isolation, assert
	// the subprocess actually ran in the job's declared Workspace (not
	// wherever the parent process happened to be), and exercise the
	// UnknownEvent forward-compat path on a real, non-fixture event.
	if fixture == "echoargs" {
		cwd, err := os.Getwd()
		if err != nil {
			fmt.Fprintln(os.Stderr, "fakeclaude: getting cwd:", err)
			os.Exit(2)
		}
